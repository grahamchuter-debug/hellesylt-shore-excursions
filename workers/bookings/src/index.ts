/**
 * Hellesylt request-to-book Worker (Briksdal Glacier Discovery).
 *
 ...
 * LIVE_PAYMENTS_CODE_ENABLED is true for H-4 public launch (still needs unlock + bookings).
 * TEST and LIVE keys are never interchangeable.
 * Do not alter Belize / Olden stub fixtures / other destinations.
 */
...
async function handlePreviewRequest(request: Request, env: Env): Promise<Response> {
    ...
    return jsonResponse(
      {
        ...
        message:
          "Live Stripe is not enabled for this Hellesylt Worker. Charge-then-refund still requires an explicit production configuration. No payment was taken.",
      },
      503,
    );
  }
  ...
  const product = findHellesyltBookingProduct(body.productId);
    ...
    return jsonResponse(
      {
        ok: false,
        code: "ELIGIBILITY",
        message: "Please confirm you understand the excursion requirements before requesting.",
      },
      400,
    );
  }
  ...
  const reference = createBookingReference(hellesyltBookingCore);
  ...
  const destinationBrand = destinationBrandFromCore(hellesyltBookingCore);
