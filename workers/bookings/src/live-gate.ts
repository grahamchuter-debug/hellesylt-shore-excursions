 ...
 */
/** H-1 scaffold: live code flag OFF until Graham unlock. */
...
export function liveReadinessGaps(
  ...
  if (!(env.SITE_BASE_URL || "").includes("hellesyltshoreexcursions.com")) gaps.push("site_base_url");
  if (!product || supplierRoutingIsPlaceholder(product)) gaps.push("supplier_routing");
  if (!product || product.capacity.maxGuestsPerBookingSource !== "approved") gaps.push("capacity_unapproved");
  return gaps;
}
...
export function liveCheckoutBlock(
  ...
  if (!LIVE_PAYMENTS_CODE_ENABLED) {
    return {
      ...
      message: "Live Stripe is not enabled for Hellesylt. The live code flag is off. No payment was taken.",
    };
  }
  ...
  if (!bookingsAreEnabled(env)) {
    return {
      ...
      message: "Hellesylt online booking is switched off. No payment was taken.",
    };
  }
  if (!env.DB) {
    return {
  ...
  if (!site.includes("hellesyltshoreexcursions.com")) {
    return {
      code: "SITE_BASE_URL",
      message: "Live Checkout requires SITE_BASE_URL on hellesyltshoreexcursions.com. No payment was taken.",
    };
  }
  if (!product) {
    return {
  ...
  if (product.capacity.maxGuestsPerBookingSource !== "approved") {
    return {
      code: "CAPACITY_UNAPPROVED",
      message:
        "Online guest capacity is not commercially approved. Live Checkout will not start. No payment was taken.",
    };
  }
  return null;
}
