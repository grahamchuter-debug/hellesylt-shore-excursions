/**
 * Temporary Hellesylt visual QA harness.
 */
...
try {
  ...
  puppeteer = require(
    path.resolve(root, "../eidfjord-shore-excursions/node_modules/puppeteer-core"),
  );
}
...
const pages = [
  ...
  { path: "/excursions/mount-stranda-panoramic-views", label: "mount-stranda" },
  { path: "/excursions/private-panoramic-geiranger-lunch", label: "geiranger-private" },
  { path: "/hellesylt-port-guide", label: "port-guide" },
  { path: "/one-day-in-hellesylt", label: "one-day" },
  { path: "/is-hellesylt-worth-visiting", label: "worth-visiting" },
  { path: "/best-time-to-visit-hellesylt", label: "best-time" },
  { path: "/ship-schedule", label: "schedule-hub" },
  { path: "/ship-schedule/july-2026", label: "month-busy" },
  { path: "/ship-schedule/april-2027", label: "month-light" },
  ...
  for (const vp of viewports) {
    ...
    for (const route of pages) {
      ...
      const metrics = await page.evaluate((isHome) => {
        ...
        const sibling = /Lysefjord|Pulpit Rock|Bryggen|Nidaros|Bakklandet|Flamsbana|Stegastein|Voringsfossen/i.test(
          body.innerText || "",
        );
        const gatewayIdentity =
          isHome && /Gateway to Geiranger/i.test(body.innerText || "");
        const isoRaw = /\b20\d{2}-\d{2}-\d{2}\b/.test(body.innerText || "");
        return { overflowX, bookHits, emailLinks, h1, sibling, gatewayIdentity, isoRaw };
      }, route.path === "/");
      ...
      if (
        metrics.emailLinks.some((h) =>
          (h || "").includes("hello@hellesyltshoreexcursions.com"),
        )
      ) {
        note(
          "FAIL",
      ...
      if (metrics.gatewayIdentity) {
        note("FAIL", `Gateway to Geiranger identity on homepage @${vp.name}`);
      }
